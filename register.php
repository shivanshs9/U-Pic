...
$dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME)
	or die("<p class='error'>Error Connecting to MYSQLi server...</p>");

if (isset($_POST['register'])) {
	// Grab the profile data from the POST
	$username = mysqli_real_escape_string($dbc, trim($_POST['name']));
	$password1 = mysqli_real_escape_string($dbc, trim($_POST['pass']));
	$password2 = mysqli_real_escape_string($dbc, trim($_POST['pass2']));
